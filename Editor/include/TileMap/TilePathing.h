...
public:
	struct Cell {
		...
		Cell() = default;
		Cell(glm::uvec2 coords, uint32_t cost) :
...
public:
	...
	Zone FindMovementZone(glm::uvec2 coords, uint32_t movementSteps);
...
private:
	...
	uint32_t Heuristic(glm::uvec2 start, glm::uvec2 end) const;
